...
impl NegotiatedCipherSuite {
    ...
    pub fn new(algorithm: &good_internal_aead::Algorithm, secret: GoodPrk) -> Option<(Self, HeaderKey)> {
        Some(match algorithm {
            _ if algorithm == &good_internal_aead::AES_256_GCM => {
                let (cipher_suite, header_key) = TLS_AES_256_GCM_SHA384::new(secret);
                (cipher_suite.into(), header_key)
            }
            _ if algorithm == &good_internal_aead::CHACHA20_POLY1305 => {
                let (cipher_suite, header_key) = TLS_CHACHA20_POLY1305_SHA256::new(secret);
                (cipher_suite.into(), header_key)
            }
            _ if algorithm == &good_internal_aead::AES_128_GCM => {
